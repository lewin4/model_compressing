...
import torch
from src.compressed_layers.CompressedConv2d import CompressedConv2d
from src.utils.models import channel_selection
import numpy as np
...
def compute_flops(module, inp, out):
    if isinstance(module, nn.Conv2d):
        ...
        return compute_CompressedConv2d_flops(module, inp, out)
    elif isinstance(module, channel_selection):
        return 0
    else:
...
def compute_CompressedConv2d_flops(module, inp, out):
    # Can have multiple inputs, getting the first one
    assert isinstance(module, CompressedConv2d)
    if len(inp) > 1:
        for in_p in inp:
            if isinstance(in_p, torch.Tensor):
                inp = in_p
                break
    assert len(inp.size()) == 4 and len(inp.size()) == len(out.size())
    ...
    batch_size = inp.size()[0]
    in_c = inp.size()[1]
    k_h, k_w = module.kernel_height, module.kernel_width
    out_c, out_h, out_w = out.size()[1:]
    groups = module.groups
